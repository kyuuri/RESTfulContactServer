...
public class JpaContactDao implements ContactDao {
	...
	private void createTestContact( ) {
		...
		if (find(id) == null) {
			Contact test = new Contact("Test contact", "Joe Experimental", "none@testing.com","1234");
			test.setId(id);
			save(test);
		}
		id++;
		if (find(id) == null) {
			Contact test2 = new Contact("Another Test contact", "Testosterone", "testee@foo.com","4321");
			test2.setId(id);
			save(test2);
		}
	}
	...
	public List<Contact> findAll() {
		Query query = em.createQuery("SELECT o FROM Contact o");
	...
	public boolean update(Contact update) {
		...
		try{
			tx.begin();
			Contact contact = this.find(update.getId());
			if(contact == null){
				throw new IllegalArgumentException("Can't update a null contact");
			}
